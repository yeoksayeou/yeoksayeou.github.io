...
# Updated pattern to match both Korean and English metadata section in the article files
metadata_pattern = re.compile(
    r'(?:잡지명|Magazine Title:)\s+(.*?)\n'
    r'(?:발행일|Publication Date:)\s+(.*?)\n'
    r'(?:기사제목|Article Title:)\s+(.*?)(?:\n(?:필자|Author:)\s+(.*?))?(?:\n(?:기사형태|Article Type:)\s+(.*?))?(?:\n\n)(.*)',
    re.DOTALL
)
    ...
    for filename in txt_files:
        try:
            ...
            with open(file_path, encoding="utf-8") as f:
                ...
                # Initialize all fields to ensure they're present in the JSON even if empty
                article.update({
                    'magazine': '',
                    'date': '',
                ...
                if metadata_match:
                    # Extract metadata, updating the pre-initialized fields
                    content = metadata_match.group(6).strip()
                    if is_incomplete:
                        content = "Translation failed. Often because the file was too long for the output context limits.\n\n" + content
                    
                    article.update({
                        'magazine': metadata_match.group(1).strip(),
                        'date': metadata_match.group(2).strip(),
                        'title': metadata_match.group(3).strip() if metadata_match.group(3) else article['title'],
                ...
                # For files without metadata match, we already set defaults above
                # Update content for incomplete files without metadata match
                elif is_incomplete:
