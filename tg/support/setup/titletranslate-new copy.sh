...
#######################################################################
# Title Translation Batch Processing Script
#######################################################################
#
# Description:
#   This script processes a batch of files with names starting with "out_"
#   in the current directory. It uses an LLM (Language Learning Model) to 
#   process each file according to instructions in prompt.txt, and saves
#   the results with "-en.txt" appended to the original filename.
#
# Requirements:
#   - An 'llm' command in the PATH (likely a CLI tool for accessing LLMs)
#   - A 'prompt.txt' file in the current directory containing the prompt/instructions
#   - Files with names starting with "out_" in the current directory to process
#   - The 'bc' command for floating-point calculations (optional, falls back to integer math)
#
# Features:
#   - Processes multiple files in sequence with progress tracking
#   - Implements timeout (10 minutes) for each processing task
#   - Retries failed processing up to 3 times per file
#   - Logs errors to a separate 'errors.txt' file
#   - Displays timing statistics (time per file, average time, estimated completion time)
#
# Usage:
#   ./titletranslate-new.sh
#
# Output:
#   - Processed files named as "{original-filename}-en.txt"
#   - Real-time progress information on stdout
#   - Error information in errors.txt
#
#######################################################################
...
format_time() {
    ...
    [ -z "$seconds" ] || [ "$seconds" -ne "$seconds" ] 2>/dev/null && seconds=0
...
for file in out_*; do
    ...
    # Convert floating point to integer by stripping decimal portion
    avg_time_int=${avg_time%.*}
    # Default to 0 if empty (happens when avg_time < 1)
    [ -z "$avg_time_int" ] && avg_time_int=0
    avg_time_fmt=$(format_time_short $avg_time_int)
    # Handle estimated time similarly
    est_remaining_int=${est_remaining_time%.*}
    [ -z "$est_remaining_int" ] && est_remaining_int=0
    est_remaining_fmt=$(format_time $est_remaining_int)
