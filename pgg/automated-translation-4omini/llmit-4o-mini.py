...
EXCLUDED_DIRS = ["automated-translation","done","4o-mini-translation", "search-files"]
EXCLUDED_FILES = ["full.txt", "index.html", "index.md", "word_frequency.txt", "word_frequency.txv"]
...
def process_file(input_path, output_path, current_file, total_files, total_processing_time, processed_count, remaining_files):
    ...
    while retries < MAX_RETRIES:
        ...
        cmd = f'(cat "{safe_input_path}" | llm "$(cat {PROMPT_FILE})" -m 4o-mini; echo "\\n\\n---------------------------\\n\\n"; cat "{safe_input_path}") > "{safe_output_path}"'
        # log(cmd)
